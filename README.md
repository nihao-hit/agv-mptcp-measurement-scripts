...
+ 定位掉线与通信时延大的问题．
+ 六大块内容
  1. 基站覆盖分析：刻画基站覆盖情况，发现基站覆盖空白；统计关联基站的最小SNR与未关联基站的最大SNR．
...
4. 数据解析
   ...
   2. 一次测量结束时也会使用`iw config`工具打印当前连接基站信息，提取此数据缓解conn文件测量空洞的问题，填补时刻为测量结束时刻etime。
...
统计关联基站的最小SNR与未关联基站的最大SNR．

1. 非图表型统计数据：conn数据总数，WLAN0数据总数，WLAN0过滤后数据总数，WLAN1数据总数，WLAN1过滤后数据总数，start，end，duration，时间粒度，WLAN0最小SNR，WLAN0最大SNR，WLAN1最小SNR，WLAN1最大SNR，WLAN0 Not-Associated数据总数，WLAN1 Not-Associated数据总数，WLAN0未关联基站最大SNR1，WLAN0未关联基站最大SNR2，WLAN1未关联基站最大SNR1，WLAN1未关联基站最大SNR2．
2. 粗粒度分析
   1. 无线网卡连接基站的SNR分布CDF，
   2. 无线网卡未连接基站时观测到的基站最大SNR分布CDF
   3. 基站覆盖热力图
   4. 有效基站覆盖热力图
   5. 基站覆盖空白热力图
...
2. 粗粒度分析
   ...
   5. 漫游SNR增益CDF：SNR值可简单使用关联基站时刻的SNR值。
      + 不同时长分类的SNR增益没有明显区别，不需要研究
      + 不同类型分类的SNR增益理论上有明显区别
...
3. 细粒度分析
   ...
   2. 漫游事件SNR分析图
      ...
      2. 通常flag=1类型的漫游导致SNR增益较小或为负，且flag=1的漫游后往往紧接着发生flag=0的漫游，间隔在1s以内．
      3. 如何从1号车WLAN0的2800多次漫游事件中选择**表现力最好**且能**发现问题**的图片
         1. 手动过滤：可保证发现问题
         2. 程序过滤：设置过滤条件例如数据完整性，保证表现力．
4. 802.11漫游协议：802.11k、802.11r、802.11v
