...
# 返回填补统计信息
def fillPing127Forward(sList, startTime, fillDir):
    ...
    while i < len(sList):
        if sList[i].W1pingrtt == 0:
            txTime = startTime + i
            j = 1
            while i < len(sList) and sList[i].W1pingrtt == 0:
                if j <= 6:
                    # 使用前5s的均值填补，5s是不是偏多，3s会不会更好一些
                    k = i-1; meanRttCount = 5
                    meanRtt = 0
                    while k >= 0 and meanRttCount > 0 and sList[k].W1pingrtt < 7000:
                        meanRtt += sList[k].W1pingrtt
                        k -= 1
                        meanRttCount -= 1
                    meanRtt /= 5-meanRttCount if meanRttCount != 5 else 1
                    # 缺失1s的填补值偏大，但是1s的缺失有可能是偶发现象又居多，修正一下
                    if j == 1:
                        sList[i].W1pingrtt = int(meanRtt)
                    else:
                        sList[i].W1pingrtt = int(meanRtt) + random.randint(0, 1000)
                else:
                    sList[i].W1pingrtt = j*1000
                i += 1
                j += 1
                # 记录填补多少次
                count += 1
    ...
    fileName = 'fillPing127Forward-{}.txt'.format(count)
...
def fillPing127Backward(sList, startTime, fillDir):
    ...
    for i in range(len(sList)):
        if sList[i].W1pingrtt > 1e-6:
            if flag:
                for j in range(1, i - prev):
                    # 离散化填补值，[-500ms, 500ms]
                    tmpRtt = j*1000
                    sList[i - j].W1pingrtt = random.randrange(tmpRtt-500, tmpRtt+500)
                    count += 1
    ...
    fileName = 'fillPing127Backward-{}.txt'.format(count)
    with open (os.path.join(fillDir, fileName), 'w') as f:
        f.write('{},\t{},\t{}\n'.format('startTime', 'endTime', 'duration'))
        for tpl in fillTimes:
            f.write('{},\t{},\t{}\n'.format(
